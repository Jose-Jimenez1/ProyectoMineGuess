...
namespace MVP_ProyectoFinal.Models
{
    public static class RepositorioEntidades
    {
        ...
        public static List<Entidad> ObtenerTodos() => _entidades;

        public static Entidad? ObtenerPorNombre(string nombre)
        {
            return _entidades.FirstOrDefault(e => e.Nombre.Equals(nombre, StringComparison.OrdinalIgnoreCase));
        }

        public static Entidad? ObtenerEntidadAleatoria()
        {
            if (_entidades.Count == 0) return null;
            int index = _random.Next(_entidades.Count);
            return _entidades[index];
        }
    }
}